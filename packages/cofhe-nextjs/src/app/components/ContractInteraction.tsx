...
export function ContractInteraction() {
  ...
  const [hoverInfo, setHoverInfo] = useState<string>(
    "Hover over button to get information"
  );
  ...
  const buttonDescriptions = {
    ...
    decrypt:
      "Request decryption of the counter, this will perform asynchronous decryption and store the value on chain once it is ready",
    getEncrypted:
      "This will retrieve the encrypted counter value and unseal a permit. This will not trigger decryption transaction",
  ...
  const { data: decryptedCount, refetch: refetchDecryptedCount } =
    useReadContract({
  ...
  const { data: encryptedCount, refetch: refetchEncryptedCount } =
    useReadContract({
      address: contractAddress as `0x${string}`,
      account: address,
      abi: CONTRACT_ABI,
